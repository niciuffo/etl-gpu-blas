...
void bench_cudnn_bias_batch_sum_4d_tensor_cores(size_t D0, size_t D1, size_t D2, size_t D3, size_t repeat = 100) {
    const size_t SIZE = D0 * D1 * D2 * D3;
    ...
    cudnn_check(cudnnCreateConvolutionDescriptor(&cudnnConvDesc));

    // Set the math type to allow cuDNN to use Tensor Cores:
    cudnn_check(cudnnSetConvolutionMathType(cudnnConvDesc, CUDNN_TENSOR_OP_MATH_ALLOW_CONVERSION));
    ...
    report("cudnn_bias_batch_sum_4d_tensor_cores", t0, repeat, SIZE);
...
void bench_cudnn_bias_batch_sum_4d_tensor_cores() {
    bench_cudnn_bias_batch_sum_4d_tensor_cores(256, 24, 16, 10);
    bench_cudnn_bias_batch_sum_4d_tensor_cores(256, 24, 16, 100);
    bench_cudnn_bias_batch_sum_4d_tensor_cores(256, 24, 16, 1000);
    //bench_cudnn_bias_batch_sum_4d_tensor_cores(256, 24, 16, 10000); // OOM with 4GB
    std::cout << std::endl;
}
...
int main(int argc, char* argv[]){
    ...
    if (sub == "bias_batch") {
        ...
        bench_cudnn_bias_batch_sum_4d_tensor_cores();
    }
}
